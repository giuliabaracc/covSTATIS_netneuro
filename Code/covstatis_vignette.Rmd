...
library(sp)         #convex hulls in compute_task_diff()
library(ggplot2)
```

# Setup

Retrieve data from OSF. Download nback (0-,1-,2-back) data from: https://osf.io/hnj7s/. You should have 144x3 txt files (nx3 task conditions). Make sure to download the following csv file "participants_ages.csv" as well from: https://osf.io/dm23b/. Make sure txt files, participant info and the atlas labels csv files are stored in the Data folder. 

## Read in the data

The first step is to read in our connectivity matrices to form a 3D array (= 'X' here below) for analysis with covSTATIS. In this vignette the 3D array dimensionality is 100 x 100 x 99 (ROI x ROI x N participants) per task condition (3 in total). 
...
Let's first visualize the compromise (i.e., the optimal linear combination of the connectivity matrices)

```{r}
# heatmap.2(covstatis_res$res4Splus$Splus, 
...
superheat(covstatis_res$res4Splus$Splus,
          ...
          # heat.col.scheme = "RdBu",
          heat.lim = c(-0.1,0.1),
          heat.pal = rev(brewer.pal(200, "RdBu")),
...
```
Next, the compromise undergoes the eigendecomposition, which gives a scree plot as follows:
...
Let's now reconstruct a heatmap to explore the dimensions from covSTATIS. Let's start by visualizing the first two components (LV1, LV2).
...
## create labels for the component space
comp.lab <- createxyLabels.gen(x_axis = h_axis, y_axis = v_axis,
...
print(compromise_graph_out$zeMap_background + compromise_graph_out$zeMap_dots + comp.lab)
```
...
print(compromise_network_graph_out$zeMap + compromise_network_graph_out$zeMap_dots + comp.lab)
```
...
print(compromise_graph_out$zeMap_background + compromise_graph_out$zeMap_dots + 
        compromise_network_graph_out$zeMap_dots + compromise_network_graph_out$zeMap_text + comp.lab)
```
...
print(compromise_graph_out$zeMap_background + compromise_graph_out$zeMap_dots +
  compromise_pfs$mapColByBlocks + ggtitle('Compromise map - Partial factor scores by task design') + comp.lab)
```
...
print(compromise_network_graph_out$zeMap + compromise_pfs_network$mapColByBlocks + ggtitle('Network level compromise map - Partial factor scores by task design') + comp.lab)
```
...
for(i in 1:length(ids)){
  ...
  for(n in 1:length(network_labels)){
    hull_pts <- peelZeHull(cbind(this_lv1_network[n,], this_lv2_network[n,])) #here you can play with the flag "percentage" and include either all points (like done here) or e.g., 95% of them in which case you'd write "percentage=0.95"
